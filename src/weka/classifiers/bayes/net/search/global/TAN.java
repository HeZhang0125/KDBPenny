...
public class TAN extends GlobalScoreSearchAlgorithm implements
  ...
  public void buildStructure(BayesNet bayesNet, Instances instances)
	...
	public static void main(String[] argv) throws Exception {
		...
		FileReader frData = new FileReader("zoo.arff");
		...
		TAN tree = new TAN();
		tree.buildStructure(net, data);
		for(int i = 0; i < net.getNrOfNodes();i++)
			for(int j =0; j < 2; j++)
				System.out.println(net.getNodeName(i)+"  "+net.getParent(i, j));
	
		
		Evaluation eval = new Evaluation(data);
//		eval.crossValidateModel(net, data, 5, new Random(1));
//		System.out.println(eval.rootMeanSquaredError());
		
//		double[] probs = new double[data.numClasses()];
//		probs = classifier.distributionForInstance(test);
//		for(int i = 0; i < probs.length; i++){
//			System.out.println(Double.toString(probs[i]));
//			
		}
  /**
