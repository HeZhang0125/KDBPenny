...
public class GUI extends JPanel implements LayoutCompleteEventListener {
	...
	private final int[] m_nZoomPercents = { 10, 25, 50, 75, 100, 125, 150, 175, 200, 225, 250, 275, 300, 350, 400, 450,
	...
	class Selection {
		...
		boolean contains(Rectangle rect, int iNode) {
			return rect.intersects((m_BayesNet.getPositionX(iNode)) * m_fScale,
					(m_BayesNet.getPositionY(iNode)) * m_fScale, m_nPaddedNodeWidth * m_fScale,
					m_nNodeHeight * m_fScale);
		} // contains
				...
				jBtGo.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent ae) {
						try {
							...
							// This ensures the getOptions() called by
							// GenericObjectEditor to
					...
					public void actionPerformed(ActionEvent ae) {
						try {
							...
							if (m_sFile != null && !m_sFile.equals("")) {
								...
								// no idea what the format is, so let's save it
								// as ARFF file
								if (saver == null) {
									saver = new ArffSaver();
								}
	...
	class ActionQuit extends ActionSave {
		...
		public void actionPerformed(ActionEvent ae) {
			if (m_BayesNet.isChanged()) {
				int result = JOptionPane.showConfirmDialog(null, "Network changed. Do you want to save it?",
	...
	private class GraphPanel extends PrintablePanel implements Printable {
		...
		public void paintComponent(Graphics gr) {
			...
			if (m_nSelectedRect != null) {
				g.drawRect((int) (m_nSelectedRect.x / m_fScale), (int) (m_nSelectedRect.y / m_fScale),
						(int) (m_nSelectedRect.width / m_fScale), (int) (m_nSelectedRect.height / m_fScale));
			}
		} // paintComponent
		...
		void viewCliques(Graphics g, JunctionTreeNode node) {
			...
			for (int j = 0; j < nodes.length; j++) {
				...
				for (int k = j + 1; k < nodes.length; k++) {
					g.drawLine(m_BayesNet.getPositionX(nodes[j]) + dX, m_BayesNet.getPositionY(nodes[j]) + dY,
							m_BayesNet.getPositionX(nodes[k]) + dX, m_BayesNet.getPositionY(nodes[k]) + dY);
				}
			}
			m_nClique++;
			nPosX /= nodes.length;
		...
		protected void drawNode(Graphics g, int iNode, int mode) {
			...
			if (fm.stringWidth(m_BayesNet.getNodeName(iNode)) <= m_nNodeWidth) {
				g.drawString(m_BayesNet.getNodeName(iNode),
						nPosX + m_nPaddedNodeWidth / 2 - fm.stringWidth(m_BayesNet.getNodeName(iNode)) / 2,
			...
			if (m_bViewMargins) {
				...
				for (int iValue = 0; iValue < P.length; iValue++) {
					...
					g.drawString(m_BayesNet.getNodeValue(iNode, iValue) + " " + sP,
							nPosX + m_nPaddedNodeWidth + (int) (P[iValue] * 100), nPosY + iValue * 10 + 10);
	...
	void renameValue(int nTargetNode, String sValue) {
		String sNewValue = JOptionPane.showInputDialog(null, "New name for value " + sValue,
				"Node " + m_BayesNet.getNodeName(nTargetNode), JOptionPane.OK_CANCEL_OPTION);
	...
	void addValue() {
		...
		String sNewValue = JOptionPane.showInputDialog(null, "New value " + sValue,
				"Node " + m_BayesNet.getNodeName(m_nCurrentNode), JOptionPane.OK_CANCEL_OPTION);
		if (sNewValue == null || sNewValue.equals("")) {
			return;
		}
		try {
