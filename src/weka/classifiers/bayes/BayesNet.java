...
WeightedInstancesHandler, Drawable, AdditionalMeasureProducer {
	...
	/**
	 * Variation of initStructure() to be called from wdBayes.java
	 * @throws Exception
	 */
	public void initStructure(int n) throws Exception {

		// reserve memory
		m_ParentSets = new ParentSet[n];

		for (int iAttribute = 0; iAttribute < n; iAttribute++) {
			m_ParentSets[iAttribute] = new ParentSet(n);
		}
	} // initStructure
	...
	public static void main(String[] argv) throws Exception{
		...
		for(int i = 0; i < net.getNrOfNodes();i++){
			System.out.println("Parent node: "+net.getParent(i,0));
			System.out.println("num values of Parent node: "+ net.getParentCardinality(i));
			
			System.out.println("Probability: "+ net.getProbability(i, 0, 0));
		}
			
		Estimator[][] est = net.getDistributions();
		for(int i = 0; i < est.length;i++)
			for(int j = 0; j < est[i].length;j++)
				System.out.println(est[i][j]);
		
		for(int i = 0; i < net.getNrOfNodes()-1;i++){
			System.out.println("node"+i);
			for(int j = 0; j < net.getCardinality(net.getParent(i, 0));j++){
					System.out.println(net.getProbability(i, 0, j));
			}
		}
		System.out.println("MLE score: "+ net.measureMDLScore());
		System.out.println(net.toString());
		
		Instance test= data.instance(0);
		double[] probs = net.distributionForInstance(test);
