...
public class NaiveBayes extends AbstractClassifier
	...
	public void buildClassifier(Instances instances) throws Exception {
		...
		// m_Distributions is P(C|X), m_ClassDistribution is P(C)
		...
		while (enu.hasMoreElements()) {
			...
			double numPrecision = DEFAULT_NUM_PRECISION;// 0.01
			if (attribute.type() == Attribute.NUMERIC) {// 如果属性值是连续的
				
				System.out.println("连续属性。。。");
				...
				if ((m_Instances.numInstances() > 0) && !m_Instances.instance(0).isMissing(attribute)) {
					//
					double lastVal = m_Instances.instance(0).value(attribute);// lastVal最小
					...
					if (distinct > 0) {
						numPrecision = deltaSum / distinct;// numPrecision=平均差值
						System.out.println("numPrecision:"+numPrecision);
					}
				}
			}
			// 如果属性是连续的，再选择用kernel的还是normal的。 这里的构造需要用到numPrecision.
			// 具体的这个estimator如何构造，参考weka.estimator这个package。
			...
			for (int j = 0; j < m_Instances.numClasses(); j++) {
				...
				case Attribute.NUMERIC:
					if (m_UseKernelEstimator) {// 默认是false
	...
	public double[] distributionForInstance(Instance instance) throws Exception {
		...
		while (enumAtts.hasMoreElements()) {
			...
			if (!instance.isMissing(attribute)) {
				double temp, max = 0;
				for (int j = 0; j < m_NumClasses; j++) {
					System.out.print(m_Distributions[attIndex][j]);
					System.out.println("m_Distributions["+attIndex+"]["+j+"]:"+m_Distributions[attIndex][j].getProbability(instance.value(attribute)));
					...
					probs[j] *= temp;
					System.out.println("probs["+j+"]:"+probs[j]);
