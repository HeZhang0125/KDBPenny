...
public class ParallelObjectiveFunctionCLL_w extends ObjectiveFunctionCLL_w {
	...
	public FunctionValues getValues(double params[]) {
		...
		for (int th = 0; th < nThreads; th++) {
			 ...
			 * Calling thread
			 */
			Callable<Double> thread = new CallableCLL_w(algorithm.getM_Instances(), start, stop, algorithm.getNc(), nodes[th], tmpProbs[th], gs[th], algorithm.getdParameters_(), algorithm.getM_Order(),algorithm.getXxyDist_().xyDist_,algorithm);
