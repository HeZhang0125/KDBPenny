...
public class CallableCLL_w implements Callable<Double>{
	...
	public CallableCLL_w(Instances instances, int start, int stop, int nc, int n, String m_S, double[] myProbs, double[] g, wdAnJEParameters dParameters) {
	...
	public CallableCLL_w(int start, int stop, double[] myProbs, double[] g, wdAnJE algorithm, int th) {
	...
	public Double call() throws Exception {
		...
		for (int i = start; i <= stop; i++) {
			...
			for (int c = 0; c < nc; c++) {
				//dParameters.incGradientAtFullIndex(g, c, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(c));
				
				if (m_Regularization) {
					negLogLikelihood += m_Lambda/2 * dParameters.getParameterAtFullIndex(c) * dParameters.getParameterAtFullIndex(c);
					dParameters.incGradientAtFullIndex(g, c, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(c) + m_Lambda * dParameters.getParameterAtFullIndex(c));
				} else {
					dParameters.incGradientAtFullIndex(g, c, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(c));
				}
			...
			if (m_S.equalsIgnoreCase("A1JE")) {
				...
				for (int c = 0; c < nc; c++) {
					for (int att1 = 0; att1 < n; att1++) {
						...
						//dParameters.incGradientAtFullIndex(g, index, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(index));
						
						long index = dParameters.getAttributeIndex(att1, att1val, c);
						if (m_Regularization) {
							negLogLikelihood += m_Lambda/2 * dParameters.getParameterAtFullIndex(index) * dParameters.getParameterAtFullIndex(index);
							dParameters.incGradientAtFullIndex(g, index, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(index) + m_Lambda * dParameters.getParameterAtFullIndex(index));
						} else {							
							dParameters.incGradientAtFullIndex(g, index, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(index));
						}
					}
