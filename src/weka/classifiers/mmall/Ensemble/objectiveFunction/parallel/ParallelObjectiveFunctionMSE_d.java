...
public class ParallelObjectiveFunctionMSE_d extends ObjectiveFunctionMSE_d {
	...
	public ParallelObjectiveFunctionMSE_d(wdAnJE algorithm) {
		super(algorithm);
	...
	public FunctionValues getValues(double params[]) {
		...
		for (int th = 0; th < nThreads; th++) {
			 ...
			 * Calling thread
			 */
			//Callable<Double> thread = new CallableCLL_d(algorithm.getM_Instances(), start, stop, algorithm.getNc(), algorithm.getnAttributes(), algorithm.getMS(), tmpProbs[th], gs[th], algorithm.getdParameters_());
			Callable<Double> thread = new CallableMSE_d(start, stop, tmpProbs[th], gs[th], algorithm);
