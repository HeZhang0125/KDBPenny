...
public class ObjectiveFunctionMSE_d extends ObjectiveFunction {
	...
	public ObjectiveFunctionMSE_d(wdAnJE algorithm) {
		super(algorithm);
	}
	...
	public FunctionValues getValues(double params[]) {
		...
		for (int i = 0; i < N; i++) {
			...
			for (int c = 0; c < nc; c++) {
				for (int k = 0; k < nc; k++) {
					dParameters.incGradientAtFullIndex(c, (SUtils.ind(k, x_C) - myProbs[k]) * (-1) * (SUtils.ind(c, k) - myProbs[k]) * myProbs[c]);
				}
			}
			...
			if (m_S.equalsIgnoreCase("A1JE")) {
				...
				for (int att1 = 0; att1 < n; att1++) {
					...
					for (int c = 0; c < nc; c++) {
						...
						for (int k = 0; k < nc; k++) {
							dParameters.incGradientAtFullIndex(index, (SUtils.ind(k, x_C) - myProbs[k]) * (-1) * (SUtils.ind(c, k) - myProbs[k]) * myProbs[c]);
						}
					}
				}
