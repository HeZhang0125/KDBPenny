...
public class ObjectiveFunctionCLL_w2 extends ObjectiveFunction {

	public ObjectiveFunctionCLL_w2(wdAnJE algorithm) {
		super(algorithm);
	}
	...
	public FunctionValues getValues(double params[]) {
		...
		for (int i = 0; i < N; i++) {
			...
			// -------------------------------------------------------
			for (int c = 0; c < nc; c++) {
				dParameters.incGradientAtFullIndex(c, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * (1 - n) * dParameters.getProbAtFullIndex(c));
			}
