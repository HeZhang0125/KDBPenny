...
public class ObjectiveFunctionDBL_CLL_d extends ObjectiveFunctionDBL {

	public ObjectiveFunctionDBL_CLL_d(DBL algorithm) {
		super(algorithm);
	}
	...
	public FunctionValues getValues(double params[]) {
		...
		DBLParameters dParameters = algorithm.getdParameters_();
		...
		for (int i = 0; i < N; i++) {
			...
			// -------------------------------------------------------			
			for (int c = 0; c < nc; c++) {
				algorithm.dblParameters_.incGradientAtFullIndex(c, (-1) * (SUtils.ind(c, x_C) - myProbs[c]));
			}
			...
			if (m_S.equalsIgnoreCase("A1JE")) {
				...
				for (int att1 = 0; att1 < n; att1++) {
					...
					for (int c = 0; c < nc; c++) {
						...
						algorithm.dblParameters_.incGradientAtFullIndex(index,  (-1) * (SUtils.ind(c, x_C) - myProbs[c]));
					}
				}
				...
				for (int c = 0; c < nc; c++) {
					for (int att1 = 1; att1 < n; att1++) {
						...
						for (int att2 = 0; att2 < att1; att2++) {
							...
							index = dParameters.getAttributeIndex(att1, att1val, att2, att2val, c);
							algorithm.dblParameters_.incGradientAtFullIndex(index,(-1) * (SUtils.ind(c, x_C) - myProbs[c]));
						}
					}
				}
					...
					for (int att1 = 2; att1 < n; att1++) {
						...
						for (int att2 = 1; att2 < att1; att2++) {
							...
							for (int att3 = 0; att3 < att2; att3++) {
								...
								index = dParameters.getAttributeIndex(att1, att1val, att2, att2val, att3, att3val, c);
								algorithm.dblParameters_.incGradientAtFullIndex(index,(-1) * (SUtils.ind(c, x_C) - myProbs[c]));
							}
						}
					}
						...
						for (int att2 = 2; att2 < att1; att2++) {
							...
							for (int att3 = 1; att3 < att2; att3++) {
								...
								for (int att4 = 0; att4 < att3; att4++) {
									...
									index = dParameters.getAttributeIndex(att1, att1val, att2, att2val, att3, att3val, att4, att4val, c);
									algorithm.dblParameters_.incGradientAtFullIndex(index, (-1) * (SUtils.ind(c, x_C) - myProbs[c]));
								}
							}
						}
							...
							for (int att3 = 2; att3 < att2; att3++) {
								...
								for (int att4 = 1; att4 < att3; att4++) {
									...
									for (int att5 = 0; att5 < att4; att5++) {
										...
										index = dParameters.getAttributeIndex(att1, att1val, att2, att2val, att3, att3val, att4, att4val, att5, att5val, c);
										algorithm.dblParameters_.incGradientAtFullIndex(index, (-1) * (SUtils.ind(c, x_C) - myProbs[c]));
									}
								}
							}
...
//		}
		...
		return new FunctionValues(negLogLikelihood, algorithm.dblParameters_.getGradients());
	}
