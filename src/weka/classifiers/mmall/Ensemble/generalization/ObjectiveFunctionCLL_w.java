...
public class ObjectiveFunctionCLL_w extends ObjectiveFunction {
	...
	public FunctionValues getValues(double params[]) {
			...
			if (m_S.equalsIgnoreCase("A1JE")) {
				...
				for (int c = 0; c < nc; c++) {
					for (int att1 = 0; att1 < n; att1++) {						
						...
						int index2 = dParameters.getAttributeIndexIndex(att1);
						
						if (m_Regularization) {
							negLogLikelihood += m_Lambda/2 * dParameters.getParameterAtFullIndex(index2) * dParameters.getParameterAtFullIndex(index2);
							dParameters.incGradientAtFullIndex(index2, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(index) + m_Lambda * dParameters.getParameterAtFullIndex(index2));
						} else {							
							dParameters.incGradientAtFullIndex(index2, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(index));
						}
					}
				}
				...
				for (int c = 0; c < nc; c++) {
					for (int att1 = 1; att1 < n; att1++) {
						...
						for (int att2 = 0; att2 < att1; att2++) {
							...
							int index2 = dParameters.getAttributeIndexIndex(att1,att2);
					...
					for (int att1 = 2; att1 < n; att1++) {
						...
						for (int att2 = 1; att2 < att1; att2++) {
							...
							for (int att3 = 0; att3 < att2; att3++) {
								...
								int index2 = dParameters.getAttributeIndexIndex(att1,att2,att3);
						...
						for (int att2 = 2; att2 < att1; att2++) {
							...
							for (int att3 = 1; att3 < att2; att3++) {
								...
								for (int att4 = 0; att4 < att3; att4++) {
									...
									int index2 = dParameters.getAttributeIndexIndex(att1,att2,att3,att4);
							...
							for (int att3 = 2; att3 < att2; att3++) {
								...
								for (int att4 = 1; att4 < att3; att4++) {
									...
									for (int att5 = 0; att5 < att4; att5++) {
										...
										int index2 = dParameters.getAttributeIndexIndex(att1,att2,att3,att4,att5);

										if (m_Regularization) {
											negLogLikelihood += m_Lambda/2 * dParameters.getParameterAtFullIndex(index) * dParameters.getParameterAtFullIndex(index2);
											dParameters.incGradientAtFullIndex(index2, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(index) + m_Lambda * dParameters.getParameterAtFullIndex(index2));
										} else {											
											dParameters.incGradientAtFullIndex(index2, (-1) * (SUtils.ind(c, x_C) - myProbs[c]) * dParameters.getProbAtFullIndex(index));
										}
		...
		if (algorithm.isM_MVerb()) {
			System.out.print(Utils.doubleToString(negLogLikelihood, 6, 4) + ", ");			
		}
