...
public class wdBayesParametersTreeTAN extends wdBayesParametersTree {

	public wdBayesParametersTreeTAN(Instances instances, int[] paramsPerAtt, int[] m_Order, int[][] m_Parents,
			int m_P) {
				...
				if (pt.children[j] != null) {
					...
					for (int y = 0; y < nc; y++) {
						...
						for (int uval = 0; uval < m_ParamsPerAtt[u]; uval++) {
							...
							// System.out.print(tempArray[y][uval]+" ");
							a += tempArray[y][uval];
